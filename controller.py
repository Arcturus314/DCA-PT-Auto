...
def fetch_leaf_dict(path, indict):
    ...
    if type(indict) != dict:
        ...
        return fetch_leaf_dict(path[1:], indict[path[0]])
...
def compare_against_threshold(rgba):
    # True if green component > 128, red < 128
    if rgba[1] > 128 and rgba[0] < 128:
        return True
    return False
...
class OmronVNCInterface:
    ...
    def vnc_click_location(self, x, y):
        ...
        while (1):
            try:
                ...
                time.sleep(0.3) # need time for the PT to respond
                return
            except TimeoutError:
    ...
    def vnc_read_location(self, x, y):
        ...
        # fetching screenshot
        self.client.captureScreen(self.screenshot_filename)
